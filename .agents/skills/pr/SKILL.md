...
For Topo, `scripts/dev-local.sh up` performs the CLI preflight; there is no running
service or login session. Exercise CLI behavior with synthetic data in a temporary
directory, and run the complete codified gate with `scripts/verify.sh`.
...
Get a **reviewable link** for the success video or CLI transcript. GitHub can't
play video inline via automation, so upload it somewhere with a stable URL — a
dedicated `pr-evidence` GitHub prerelease (`gh release upload`), a bucket, or CI
artifacts — and link it.
